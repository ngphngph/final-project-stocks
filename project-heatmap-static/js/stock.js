...
async function loadStockDetail() {
    ...
    // 靜態版沒有伺服器端模板可以注入 data-stock-id，改成從網址 query string 讀取
    // 例如 stock.html?id=74
    const stockId = new URLSearchParams(window.location.search).get('id');

    if (!stockId) {
        infoEl.textContent = 'Missing stock id in URL (expected ?id=...).';
        return;
    }

    const response = await fetch(`${API_BASE}/api/ohlc?id=${stockId}`);
    if (!response.ok) {
